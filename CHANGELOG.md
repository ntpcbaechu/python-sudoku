...
### 🎨 V1.1.0 - 點擊體驗與游標優化
- **問題痛點**：點擊 `<input>` 格子時會出現閃爍的文字游標（`I`），且選中樣式不明顯，體驗不夠像遊戲。
- **修改亮點**：
...
- **修改亮點**：
  1. **重構 CSS 格線**：採用獨立 `gap: 1px` 與灰色背景形成清晰 $1\text{px}$ 細線，精準繪製 $3\times3$ 九宮格黑粗框。
  2. **同數字全盤高亮**：允許點擊預設（黑色）與已填數字，點擊時全盤所有相同數字同步套用淡黃色背景（`#fff9c4`）。
  3. **新增超困難 (Expert) 難度**：於選單中新增第 4 種高難度題庫。
...
- **修改亮點**：
  1. **左右橫向並排**：將 1~9 數字鍵盤移至棋盤右側（1欄9列單直欄排列），畫面收納於單一螢幕免滾輪。
  2. **開始畫面流程**：預設隱藏棋盤，強迫玩家先輸入「姓名」與下拉選擇「難度」，按下 `▶ 開始遊戲` 後才展開棋盤與啟動計時。
  3. **開始後自動隱藏設定欄**：點擊開始後隱藏頂部輸入框，讓介面保持極簡專注。
  4. **右側新增功能鍵**：新增 `⏸️ 暫停`（含半透明遮罩）與 `🔄 重來` 功能。

---

### 🏳️ V1.4.0 - 選單體驗與極簡視覺優化 (UI Polish & Give Up)
- **問題痛點**：全盤綠色塊容易導致視覺疲勞；完成遊戲後無法直接返回選單。
- **修改亮點**：
  1. **新增「🏳️ 放棄」按鈕**：位於右側按鈕區，點擊確認後可隨時結束當前對局並返回主畫面。
  2. **強迫選難度**：下拉選單預設為 `-- 請選擇難度 --`，防止玩家誤以預設難度直接開始。
  3. **通關自動返回**：跳出「🎉 恭喜通關」彈窗後，點擊「確定」自動重置並優雅返回主選單。
  4. **極簡視覺風**：移除大面積常駐綠色區塊，棋盤保持白底乾淨質感，全靠右側按鈕鎖定提示進度。
  5. **程式碼全註解**：補齊 HTML / CSS / JavaScript 核心邏輯逐行中文備註，大幅提升可讀性與後續維護性。
