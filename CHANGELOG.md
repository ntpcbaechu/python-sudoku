...
#### 🌟 V9.0.0 - 電腦版經典三欄精緻排版與低調切換按鈕
- **Commit Message**: `Upgrade to V9.0: classic three-column layout, centered button groups, and low-profile centered footer view mode toggle`
- **修改亮點**：
  1. **電腦版三欄式結構**：左側為 9x9 棋盤，中間為 1~9 直排且平均分散的數字鍵，右側則為獨立的功能區塊。
  2. **右側功能群組化**：暫停按鈕置頂、復原/清除/放棄群組置中對齊、排行榜按鈕置底。
  3. **低調底部的切換按鈕**：將原本較為突兀的右上角切換顯示模式按鈕，移至首頁與遊戲畫面的最下方正中央，字體變小且顏色低調，不搶奪遊戲視覺焦點。
  4. **手機版保持獨立流暢**：手機排版維持原有的直向舒適佈局，確保各尺寸螢幕都能享有最佳體驗。